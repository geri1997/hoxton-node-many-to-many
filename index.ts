...
app.use(
    cors({
        origin: 'https://hinder.netlify.app/',
    })
);
...
app.patch('/interviewers/:id', (req, res) => {
    ...
    if (!getInterviewerById(id))
        return res
            .status(404)
            .send({ error: `Interviewer with id ${id} doesn't exist` });

    if (typeof name === 'string') updateRow('interviewers', 'name', name, id);
    if (typeof email === 'string')
        updateRow('interviewers', 'email', email, id);
...
app.patch('/applicants/:id', (req, res) => {
    ...
    if (!getApplicantById(id))
        return res
            .status(404)
            .send({ error: `Applicant with id ${id} doesn't exist` });
